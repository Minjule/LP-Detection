...
image = cv2.imread("testimages\\14.jpg")
vehicles = [2, 3, 5, 7]

if __name__ == '__main__':

    cars = coco_model(image, stream = True)
    cars_ = []
    for car in cars:
    ...
    for car_ in cars_:
        ...
        detections = license_plate_detector(car_crop, stream = True)
    ...
    turshilt = license_plate_detector(image)
    for idx, detection in enumerate(turshilt):
        boxes = detection.boxes.cpu().numpy()
        x1, y1, x2, y2, score, class_id = boxes.data[0]
        turshilt_crop = image[int(y1):int(y2), int(x1): int(x2), :]
    ...
    print(turshilt_text)
    cv2.imshow('License plate', turshilt_crop)
